...
impl RunnerReaderCacheEntry {
    fn make(runner_id: Arc<str>, mut path: std::path::PathBuf) -> Result<RunnerReaderCacheEntry> {
        ...
        if res.len() != 2 {
            anyhow::bail!("invalid runner, expected <RUNNER>:<VERSION> ; got {:?}", res);
        }
        ...
        for a in runner.actions {
            match a {
                RunnerJsonInitAction::MapFile { to, file } => {
                    ...
                    ret.actions.push(InitAction::MapFile { to, contents: Arc::from(buf) })
                },
                ...
                RunnerJsonInitAction::LinkWasm { file } => {
                    let mut buf = Vec::new();
                    zip_file.by_name(&file)?.read_to_end(&mut buf)?;
                    ret.actions.push(InitAction::LinkWasm { contents: Arc::from(buf), debug_path: format!("{}/{}", runner_id, file) })
                },
                RunnerJsonInitAction::StartWasm { file } => {
                    let mut buf = Vec::new();
                    zip_file.by_name(&file)?.read_to_end(&mut buf)?;
                    ret.actions.push(InitAction::StartWasm { contents: Arc::from(buf), debug_path: format!("{}/{}", runner_id, file) })
                },
            }
        }
        Ok(ret)
    ...
    pub fn append(&mut self, runner_id: Arc<str>, cache: &mut RunnerReaderCache) -> Result<()> {
        ...
        let cache_entry: Arc<RunnerReaderCacheEntry> = match cache_entry {
            ...
            std::collections::hash_map::Entry::Vacant(e) => {
                e.insert(Arc::new(RunnerReaderCacheEntry::make(runner_id.clone(), self.runners_path.clone())?))
            },
        }.clone();
            ...
            match act {
                InitAction::SetArgs {..} => {
                    self.was_args = match &self.was_args {
                        Some(x) => anyhow::bail!("args were set twice: old {} new {}", x, runner_id),
