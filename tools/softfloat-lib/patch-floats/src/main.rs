            ...
            wasmparser::Payload::ImportSection(section) => {
                ...
                impl Adder {
                    fn add(&mut self, name: &str, fn_type: u32) -> u32 {
                        ...
                        self.imports.import("softfloat", name, EntityType::Function(fn_type));
                        ret
                    }
                }
                        ...
                        sections.next()
                    {
                        ...
                        while !reader.eof() {
                            ...
                            match ins {
                                ...
                                Instruction::F64ConvertI32U => f.instruction(&Instruction::Call(f64_ops.unwrap().from_u32)),
                                Instruction::F64ConvertI32S => f.instruction(&Instruction::Call(f64_ops.unwrap().from_i32)),
                                Instruction::F64ConvertI64U => f.instruction(&Instruction::Call(f64_ops.unwrap().from_u64)),
                                Instruction::F64ConvertI64S => f.instruction(&Instruction::Call(f64_ops.unwrap().from_i64)),
                                Instruction::F64Add => f.instruction(&Instruction::Call(f64_ops.unwrap().add)),
                                Instruction::F64Sub => f.instruction(&Instruction::Call(f64_ops.unwrap().sub)),
                                Instruction::F64Mul => f.instruction(&Instruction::Call(f64_ops.unwrap().mul)),
                                Instruction::F64Div => f.instruction(&Instruction::Call(f64_ops.unwrap().div)),
                                Instruction::F64Le => f.instruction(&Instruction::Call(f64_ops.unwrap().le)),
                                Instruction::F64Lt => f.instruction(&Instruction::Call(f64_ops.unwrap().lt)),
                                Instruction::F64Ge => f.instruction(&Instruction::Call(f64_ops.unwrap().ge)),
                                Instruction::F64Gt => f.instruction(&Instruction::Call(f64_ops.unwrap().gt)),
                                Instruction::F64Eq => f.instruction(&Instruction::Call(f64_ops.unwrap().eq)),
                                Instruction::F64Ne => {
