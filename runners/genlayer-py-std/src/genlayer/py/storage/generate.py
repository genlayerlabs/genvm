...
def _storage_build(cls: type | _Instantiation, generics_map: dict[str, TypeDesc]) -> TypeDesc:
...
def _storage_build_generic(cls: _Instantiation, generics_map: dict[str, TypeDesc]) -> TypeDesc:
	# here args are resolved but not instantiated
	generic_params = cls.origin.__type_params__
	if len(generic_params) != len(cls.args):
		raise Exception(f"incorrect number of generic arguments parameters={generic_params}, args={cls.args}")
